    ...
    <body>
        <div class="p-5  bg-light">
            ...
            <div class="container">
                ...
                $consulta = $resultado->bind_param("ss", $socio, $libro); //Introducimos el parámetro de la ?

                $consulta = $resultado->execute(); //Ejecutamos la consulta
                ...
                if ($consulta == false) {
                    ...
                    if ($filas_afectadas != 0) {//Si la filas afectadas no son 0
                        $consulta = $resultado->bind_result($socio, $signatura, $libro, $devolucion,$fecha); //Vinculamos variable nombre(hace referencia a NOMBRE_JUGADOR de tabla jugadores) para poder utilizarla en la consulta preparada

                        echo '<table class="table table-bordered">'; //Imprimimos el inicio de la tabla
                       echo '<tr><th>NOMBRE</th><th>EJEMPLAR</th><th>TITULO</th><th>FECHA PRÉSTAMO</th><th>FECHA DEVOLUCIÓN</th></tr>';
                        while ($resultado->fetch()) {//Recorremos el resultado
                            echo "<tr><td>$socio</td><td>$signatura</td><td>$libro</td><td>$fecha</td><td>$devolucion</td></tr>";
                        }
